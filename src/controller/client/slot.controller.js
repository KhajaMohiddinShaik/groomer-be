...
const toggleSlot = async (req) => {
  ...
  if (slot.slot_salon_uuid != req.uuid)
    return errorResponse(403, messages.error.NOT_AUTHORIZED, {});
  // now we will check that slot fully booked or not if it is fully booked then client can not toggle the slot
  if (slot.slot_status == "fully booked")
    return errorResponse(403, messages.error.SLOT_FULLY_BOOKED, {});
