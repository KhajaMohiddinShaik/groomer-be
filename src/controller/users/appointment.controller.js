...
const newAppointment = async (req) => {
  ...
  const services = salon.salon_services.filter((item) =>
    receivedServices.includes(item.service_name)
  );
  const combos = salon.salon_combo_services.filter((item) =>
    receivedCombos.includes(item.combo_name)
  );
  ...
  return successResponse(201, messages.success.APPOINTMENT_INITIATED, {
    order: payment,
  });
};
...
const cancelAppointment = async (req) => {
  ...
  if (
    currentTime.isAfter(appointmentDateTime) ||
    currentTime.add(2, "hour").isAfter(appointmentDateTime)
  ) {
  ...
  const payment = await PaymentModel.findOne({
    payment_merchant_transaction_id: appointment.appointment_booking_id,
  });
  // initiating refund against payment
  const refundData = await refund(
    payment.payment_user_uuid,
    payment.payment_transaction_id,
    payment.payment_merchant_transaction_id,
  ...
  await sendConfirmationEmail(
    req.email,
    messages.subject.APPOINTMENT_CANCEL,
    "<h1>your appointment is now cancelled</h1>"
  );
...
const reScheduleAppointment = async (req) => {
  ...
  if (
    currentTime.isAfter(appointmentDateTime) ||
    currentTime.add(1, "hour").isAfter(appointmentDateTime)
  ) {
    // The appointment cannot be canceled
  ...
  const rescheduleCharge =
    (parseFloat(appointment.appointment_subtotal) * 10) / 100;
  const newAppointment = new AppointmentModel({
    appointment_uuid: uuidv4(),
    ...
    appointment_other_charges: JSON.stringify({
  ...
  return successResponse(
    201,
    messages.success.RESCHEDULE_APPOINTMENT_INITIATED,
    { order: payment }
  );
};
